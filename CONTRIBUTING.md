...
- No status checks are required yet (no CI is configured). This will be tightened as automation (Doctrine D5 — Automation Over Manual Process) comes online.
...
3. **Open a pull request into `main`.** Describe *what* changed and, per Article V (Governance Principles) of [ATLAS-000](docs/ATLAS-000-foundation-charter.md), the rationale — especially for anything touching a normative requirement.
4. **Review.** Changes to normative requirements require review for correctness, security, compatibility, maintainability, and ecosystem impact (ATLAS-000, Article V). For editorial or structural changes (typos, formatting, non-normative prose), a lighter pass is sufficient.
5. **Merge.** Squash or merge once the PR reflects the intended change and any review feedback is addressed. Delete the branch after merge.
