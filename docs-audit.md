...
| DOC-007 | Documentation validation | Whole repository | Heading fragments and prose claims are protected from drift | missing | `tools/validate_docs.py` checks target files but does not validate Markdown heading fragments or semantic claims | Evaluate anchor validation as a separate tooling change; prose remains review-owned | M |
| DOC-008 | `README.md` | Documentation Site | `mdbook serve` is the local preview path | unverifiable | `book.toml` is coherent, but mdBook was unavailable on the audit host and CI does not build the book | Add an mdBook build check only through a separately reviewed toolchain/CI change | M |
...
After DOC-001 and DOC-002 are resolved by this documentation change:
...
DOC-001 and DOC-002 are documentation corrections approved for this audit run. DOC-007 concerns validation code, and DOC-008 concerns CI/toolchain policy; both remain separate follow-up decisions because docs-loop does not edit code or silently expand automation policy.
